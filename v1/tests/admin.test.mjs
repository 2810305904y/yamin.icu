...
test("project cards keep a slightly slimmer desktop footprint", async () => {
  ...
  assert.match(rootHtml, /\/v1\/styles\.css\?v=1\.37-thought-space-wide/);
  assert.match(rootHtml, /\/v1\/scripts\/render-site\.mjs\?v=1\.37-thought-space-wide/);
  assert.match(rootHtml, /<script nomodule>/);
  assert.match(rootHtml, /window\.location\.replace\("\/v1\/"\)/);
  assert.match(v1Html, /\.\/styles\.css\?v=1\.37-thought-space-wide/);
  assert.match(v1Html, /\.\/scripts\/render-site\.mjs\?v=1\.37-thought-space-wide/);
});
...
test("thought area is prepared as a bounded motion space", async () => {
  ...
  assert.match(siteStyles, /\.thought-space\s*{[^}]*inset:\s*clamp\(42px,\s*5\.2vh,\s*58px\)[^}]*clamp\(24px,\s*3vw,\s*58px\)[^}]*clamp\(18px,\s*2\.4vh,\s*34px\)[^}]*clamp\(22px,\s*3\.2vw,\s*58px\)/s);
