...
test("version naming uses the new three-part small release format", async () => {
  ...
  assert.match(namingRules, /V1\.6\.0\s*->\s*全屏加载遮罩和读取完成后露出主页/);
  assert.match(namingRules, /V1\.6\.1\s*->\s*读取失败状态和更透明加载面板/);
  assert.match(namingRules, /V1\.6\.2\s*->\s*加载失败四秒等待和站名放大/);
  assert.match(namingRules, /V1\.6\.3\s*->\s*加载失败提示快速交接备用主页/);
  assert.match(namingRules, /V1\.6\.4\s*->\s*加载兜底动画正常释放/);
  assert.match(namingRules, /V1\.6\.5\s*->\s*加载失败淡出和站名放大/);
  assert.match(namingRules, /V1\.6\.6\s*->\s*加载失败淡出完整等待/);
  assert.match(namingRules, /V1\.6\.7\s*->\s*加载失败独立淡出阶段/);
  assert.match(namingRules, /V1\.6\.8\s*->\s*加载失败显式退出动画/);
  assert.match(namingRules, /V1\.6\.9\s*->\s*读取成功停留和加载站名层级/);
  assert.match(namingRules, /V1\.6\.10\s*->\s*加载字重回退和失败提示停留/);
  assert.match(namingRules, /V1\.6\.11\s*->\s*读取成功淡出和站名小调/);
  assert.match(namingRules, /V1\.6\.12\s*->\s*加载标题间距和站名缩小/);
  assert.match(namingRules, /V1\.6\.13\s*->\s*遮罩下预备主页和怪念头/);
  assert.match(namingRules, /V1\.6\.14\s*->\s*加载进度条首秒推进/);
